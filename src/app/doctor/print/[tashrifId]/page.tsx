...
export default function PrintTashrifPage() {
      ...
      <div style={{
        ...
        <div style={{ textAlign: 'center', marginBottom: '24px', borderBottom: '2px solid #111827', paddingBottom: '16px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px' }}>
            ...
            <img src="/urosfera-logo.png" alt="Urosfera" style={{ height: '36px' }} />
        ...
        <h3 style={{ fontSize: '14px', color: '#6b7280', textTransform: 'uppercase', marginTop: '24px', marginBottom: '8px' }}>Tekshiruv natijalari</h3>
        <div style={row}><span style={label}>Tomon / Daraja</span><span>{tashrif.tomon ?? '—'} / {tashrif.daraja ?? '—'}</span></div>
        <div style={row}><span style={label}>Vena diametri</span><span>{tashrif.vena_diametri ?? '—'} mm</span></div>
        <div style={row}><span style={label}>Reflux</span><span>{tashrif.reflux ?? '—'}</span></div>
        <div style={row}><span style={label}>Sperma konts. / harakat. / morf.</span><span>{tashrif.sperm_konts ?? '—'} / {tashrif.sperm_harakat ?? '—'}% / {tashrif.sperm_morf ?? '—'}%</span></div>
        <div style={row}><span style={label}>Testosteron / FSH / LH</span><span>{tashrif.testosteron ?? '—'} / {tashrif.fsh ?? '—'} / {tashrif.lh ?? '—'}</span></div>
        {tashrif.izoh && <div style={row}><span style={label}>Izoh</span><span style={{ textAlign: 'right', maxWidth: '60%' }}>{tashrif.izoh}</span></div>}

        <h3 style={{ fontSize: '14px', color: '#6b7280', textTransform: 'uppercase', marginTop: '24px', marginBottom: '8px' }}>Tavsiya</h3>
        <p style={{ fontSize: '15px', fontWeight: 600, margin: '0 0 12px' }}>{tashrif.tavsiya || '—'}</p>

        <h3 style={{ fontSize: '14px', color: '#6b7280', textTransform: 'uppercase', marginTop: '24px', marginBottom: '8px' }}>Dori va muolaja</h3>
        <p style={{ fontSize: '14px', whiteSpace: 'pre-wrap', margin: 0 }}>{tashrif.dori_muolaja || '—'}</p>
