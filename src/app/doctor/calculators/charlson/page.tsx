...
const KASALLIKLAR: Kasallik[] = [
  ...
  { id: 'pu', nom: 'Peptik yara', izoh: 'Anamnezdagi gastrik/duodenal yara', ball: 1 },
...
function tiriklik10Yil(cci: number) {
  if (cci === 0) return '98%'
  if (cci <= 2) return '89%'
  if (cci <= 4) return '76%'
  if (cci <= 5) return '49%'
...
const GRUPLAR = [
  { sarlavha: '1 balllik kasalliklar', ids: ['mi','chf','pvd','cvd','dem','copd','ctd','pu','mld','dm'] },
  { sarlavha: '2 balllik kasalliklar', ids: ['hemi','ckd','dmeo','tumor','leuk','lymp'] },
  { sarlavha: '3 balllik kasalliklar', ids: ['sld'] },
  { sarlavha: '6 balllik kasalliklar', ids: ['meta','aids'] },
]

export default function CharlsonPage() {
  return <Suspense fallback={null}><CharlsonIchki /></Suspense>
}

function CharlsonIchki() {
  ...
  const [yoshBal, setYoshBal] = useState<number | null>(null)
  ...
  const jami = yoshBal !== null ? kasallikBali + yoshBal : kasallikBali
  const nat = xavf(jami)
  const tayyor = yoshBal !== null

  const saqlash = async () => {
    if (!bemorId) return { error: 'Bemor tanlanmagan' }
    const kasalliklar = KASALLIKLAR.filter(k => tanlangan.has(k.id)).map(k => k.nom)
    return kalkulyatorNatijasiniSaqla({
      bemorId, kalkulyator: 'charlson', sarlavha: 'Charlson Comorbidity Index (CCI)',
      xulosa: `CCI: ${jami} — ${nat.nom} xavf | 10 yillik tiriklik: ${tiriklik10Yil(jami)}`,
      malumot: { jami, kasallikBali, yoshBal, kasalliklar },
    })
  }
      ...
      <div className="mx-auto max-w-[820px] px-8 py-8">
        ...
        <div className="rise" style={{
          ...
          <p style={{ margin: '8px 0 0', fontSize: '14px', opacity: 0.92, maxWidth: '60ch' }}>
            Operatsiya va davolash xavfini baholash uchun 19 komorbiditet va yosh asosida 10 yillik tiriklik ehtimolini hisoblash.
          </p>
        </div>
        ...
        <div className="rise" style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: '14px', padding: '18px 20px', marginBottom: '14px' }}>
          ...
          <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
            {YOSh_BALI.map(y => (
              <button key={y.label} onClick={() => setYoshBal(y.ball)} className="soft-press" style={{
        ...
        {GRUPLAR.map(g => {
          const kasalliklar = KASALLIKLAR.filter(k => g.ids.includes(k.id))
          return (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                {kasalliklar.map(k => (
                  <label key={k.id} style={{
                    ...
                    <input type="checkbox" checked={tanlangan.has(k.id)} onChange={() => toggle(k.id)}
        ...
        <div className="rise" style={{
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '16px' }}>
            <div>
              <div style={{ fontSize: '12px', color: nat.rang, textTransform: 'uppercase', fontWeight: 800, letterSpacing: '.04em', marginBottom: '6px' }}>CCI Ball (yosh bilan)</div>
            ...
            <div style={{ background: 'var(--surface)', borderRadius: '12px', padding: '16px 20px', textAlign: 'center', minWidth: '130px' }}>
              <div style={{ fontSize: '11px', color: 'var(--muted)', fontWeight: 700, marginBottom: '4px' }}>10 yillik tiriklik</div>
          ...
          {tanlangan.size > 0 && (
            <div style={{ marginTop: '12px', fontSize: '12.5px', color: 'var(--ink-soft)' }}>
              Kasalliklar bali: {kasallikBali} | Yosh bali: {yoshBal ?? 0} | Jami: {jami}
            </div>
          )}
        </div>
        ...
        <div className="rise" style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: '16px', padding: '22px 24px' }}>
          ...
          <div style={{ fontSize: '13.5px', color: 'var(--ink-soft)', lineHeight: 1.6 }}>
            <p style={{ margin: '0 0 8px' }}>
              <strong style={{ color: 'var(--ink)' }}>Charlson CCI</strong> — 1987-yilda ME Charlson tomonidan ishlab chiqilgan. Urologiyada prostatektomiya,
              nefrektomiya va boshqa operatsiyalar oldidan xavfni baholashda standart vosita.
            </p>
            <p style={{ margin: 0, fontSize: '12px', color: 'var(--muted)' }}>
              Manba: Charlson ME et al. J Chronic Dis 1987; 40:373. Faqat klinik yordamchi vosita.
            </p>
          </div>
        </div>
      </div>
