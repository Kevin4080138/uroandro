...
function ballHisob(psa: number, gleason1: number, gleason2: number, bosqich: string, pozitivYuz: number, yosh: number) {
  ...
  else if (gleason1 === 3 && gleason2 >= 4) ball += 1
...
export default function CAPRAPage() {
  return <Suspense fallback={null}><CAPRAIchki /></Suspense>
}
...
function CAPRAIchki() {
  ...
  const saqlash = async () => {
    if (!bemorId || jami === null || !natija) return { error: 'Bemor tanlanmagan yoki ma\'lumotlar to\'liq emas' }
    return kalkulyatorNatijasiniSaqla({
      bemorId, kalkulyator: 'capra', sarlavha: 'CAPRA Score',
      xulosa: `CAPRA: ${jami}/10 — ${natija.nom} (BCR-dan holi: ${natija.bcr} / 3 yil)`,
      malumot: { psa: +psa, gleason: `${gl1}+${gl2}`, bosqich, pozitivYuz: +pozitiv, yosh: +yosh, jami },
    })
  }
  ...
  const inp: React.CSSProperties = {
    ...
    fontSize: '15px', fontWeight: 700, color: 'var(--ink)', outline: 'none', boxSizing: 'border-box',
  }
      ...
      <div className="mx-auto max-w-[820px] px-8 py-8">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px', marginBottom: '16px' }}>
          <div className="rise" style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: '14px', padding: '16px 18px' }}>
            <label style={{ fontSize: '12px', fontWeight: 700, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '.04em' }}>PSA tashxis vaqtida (ng/mL)</label>
            <input type="number" value={psa} onChange={e => setPsa(e.target.value)} placeholder="8.5" step="0.1" style={inp} />
            <div style={{ fontSize: '11px', color: 'var(--muted)', marginTop: '6px' }}>&lt;6=0б | 6–9=1б | 10–19=2б | 20–35=3б | ≥36=4б</div>
          </div>
          <div className="rise" style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: '14px', padding: '16px 18px' }}>
          ...
          <div className="rise" style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: '14px', padding: '16px 18px' }}>
            <label style={{ fontSize: '12px', fontWeight: 700, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '.04em' }}>Gleason — asosiy pattern (1-5)</label>
            <input type="number" value={gl1} onChange={e => setGl1(e.target.value)} placeholder="3" min="1" max="5" style={inp} />
          </div>
          <div className="rise" style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: '14px', padding: '16px 18px' }}>
            <label style={{ fontSize: '12px', fontWeight: 700, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '.04em' }}>Gleason — ikkinchi pattern (1-5)</label>
            <input type="number" value={gl2} onChange={e => setGl2(e.target.value)} placeholder="4" min="1" max="5" style={inp} />
            <div style={{ fontSize: '11px', color: 'var(--muted)', marginTop: '6px' }}>Asosiy ≥4 = +3б | asosiy 3 + ikk. ≥4 = +1б</div>
          </div>
          <div className="rise" style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: '14px', padding: '16px 18px' }}>
            <label style={{ fontSize: '12px', fontWeight: 700, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '.04em' }}>Klinik bosqich (T)</label>
          ...
          <div className="rise" style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: '14px', padding: '16px 18px' }}>
            <label style={{ fontSize: '12px', fontWeight: 700, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '.04em' }}>Ijobiy biyopsiya yadrolarining ulushi (%)</label>
        ...
        {natija && jami !== null && (
          <div className="rise" style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '16px' }}>
              <div>
                <div style={{ fontSize: '12px', color: natija.rang, textTransform: 'uppercase', fontWeight: 800, letterSpacing: '.04em', marginBottom: '6px' }}>CAPRA Score</div>
              ...
              <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
                <div style={{ background: 'var(--surface)', borderRadius: '12px', padding: '14px 18px', textAlign: 'center', minWidth: '110px' }}>
                  <div style={{ fontSize: '11px', color: 'var(--muted)', fontWeight: 700, marginBottom: '4px' }}>BCR-dan holi</div>
        ...
        <div className="rise" style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: '16px', padding: '22px 24px' }}>
          ...
          <div style={{ fontSize: '13.5px', color: 'var(--ink-soft)', lineHeight: 1.6, display: 'flex', flexDirection: 'column', gap: '8px' }}>
            <p style={{ margin: 0 }}>
              <strong style={{ color: 'var(--ink)' }}>CAPRA</strong> — UCSF tomonidan ishlab chiqilgan. 5 klinik o&apos;zgaruvchi asosida BCR, metastaz va kasallikka oid o&apos;limni bashorat qiladi.
            </p>
            <p style={{ margin: 0, fontSize: '12px', color: 'var(--muted)' }}>
              Manba: Cooperberg MR et al. J Urol 2005; 173:1938. Faqat klinik yordamchi vosita.
            </p>
          </div>
        </div>
      </div>
