...
type KursDars = { slug: string; sarlavha: string; kategoriya: string | null; bosqich: string; qisqa: string | null; video_url: string | null; nazariya_html: string | null; test_savollar: TestSavol[]; daqiqa: number }
...
export default function UroDarsViewer() {
  ...
  const tekshir = async () => {
    ...
    await supabase.from('kurs_natijalar').upsert(
      { student_id: user.id, yonalish: YONALISH, dars_slug: slug, ball: togriSoni, jami: test.length, foiz, updated_at: new Date().toISOString() },
    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg)', color: 'var(--ink)', paddingBottom: '90px' }}>
      ...
      <div style={{ maxWidth: '760px', margin: '0 auto', padding: '20px 20px 40px' }}>
        {loading ? (
          ...
          <>
            <span style={{ display: 'inline-block', background: 'var(--accent-soft)', color: 'var(--accent)', borderRadius: '999px', padding: '3px 11px', fontSize: '11px', fontWeight: 700, marginBottom: '10px' }}>
              Urologiya
            </span>
              ...
              <div style={{ marginTop: '32px' }}>
                    ...
                    <div key={qi} style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: '14px', padding: '16px' }}>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                        {q.variantlar.map((v, vi) => {
                          ...
                          } else if (tanlangan) { bg = 'var(--accent-soft)'; bd = 'var(--accent)'; col = 'var(--accent)' }
                          return (
                ...
                {!tekshirildi ? (
                  <button onClick={tekshir} disabled={Object.keys(javoblar).length < test.length}
                    style={{
                      marginTop: '16px', width: '100%', background: 'var(--accent)', color: '#fff', border: 'none',
                  ...
                  <div style={{ marginTop: '16px', textAlign: 'center', background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: '14px', padding: '18px' }}>
                    <p style={{ margin: '0 0 4px', fontSize: '22px', fontWeight: 800, color: togriSoni === test.length ? 'var(--good)' : 'var(--accent)' }}>
