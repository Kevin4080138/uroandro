...
type KursDars = { slug: string; sarlavha: string; kategoriya: string | null; bosqich: string; qisqa: string | null; daqiqa: number }
...
function BosqichLanding() {
  ...
  useEffect(() => {
    supabase.from('kurs_darslar').select('bosqich').eq('yonalish', YONALISH).eq('faol', true).eq('bolim', 'darslar').then(({ data }) => {
  ...
  const och = (b: string) => router.push(`/student/urologiya/darslar/bosqich/${b}`)
    ...
    <div className="mx-auto max-w-[1080px] px-6 py-8">
      ...
      <div className="rise" style={{ textAlign: 'center', marginBottom: '8px' }}>
        ...
        <p style={{ margin: '10px auto 0', color: 'var(--muted)', fontSize: '13.5px', maxWidth: '520px', lineHeight: 1.6 }}>
          Har bir modul ketma-ket ochiladi — video, nazariya, materiallar va testlar bitta yo&apos;lda.
      ...
      <div style={{
        ...
        {/* ===== LEVEL 1 — FOUNDATION (bepul, ochiq) ===== */}
        <div
          onClick={() => och('oson')}
          className="rise lift"
          style={{
        ...
        >
          <div style={{ background: 'linear-gradient(135deg,#16a34a,#059669)', color: 'white', padding: '20px 22px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ fontSize: '11px', fontWeight: 900, letterSpacing: '.08em', opacity: .85 }}>LEVEL 1</span>
              <span style={{
            ...
            <p style={{ margin: 0, fontSize: '12.5px', opacity: .9 }}>Urologiyaga birinchi qadam — anatomiya, fiziologiya, simptomlar va bemorni tekshirish. Hech narsa to&apos;lamasdan darhol boshlang</p>
          </div>
        ...
        >
          ...
          <div style={{ background: 'linear-gradient(135deg,#d97706,#f59e0b)', color: 'white', padding: '20px 22px' }}>
            <span style={{ fontSize: '11px', fontWeight: 900, letterSpacing: '.08em', opacity: .85 }}>LEVEL 2</span>
            <h2 style={{ margin: '10px 0 2px', fontSize: '20px', fontWeight: 900 }}>🟡 Clinical</h2>
            <p style={{ margin: 0, fontSize: '12.5px', opacity: .92 }}>Haqiqiy shifokor kabi fikrlashni o&apos;rganasiz — infeksiyalar, tosh, BPH, prostatit, gematuriya</p>
          </div>
        ...
        {/* ===== LEVEL 3 — ADVANCED (premium dark) ===== */}
        <div
          onClick={() => och('qiyin')}
          className="rise lift"
          style={{
            background: 'linear-gradient(160deg,#101b2b 0%,#122033 55%,#0f2937 100%)',
        ...
        >
          <div style={{ padding: '20px 22px', borderBottom: '1px solid rgba(255,255,255,.08)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ fontSize: '11px', fontWeight: 900, letterSpacing: '.08em', opacity: .6 }}>LEVEL 3</span>
              <span style={{
            ...
            <p style={{ margin: 0, fontSize: '12.5px', opacity: .65 }}>
              Campbell-Walsh darajasi — onkologiya, endourologiya, neyro-urologiya, bepushtlik va real klinik qarorlar
            </p>
          </div>
...
function FlatRoyxat({ bolim }: { bolim: string }) {
  ...
  useEffect(() => {
    supabase.from('kurs_darslar').select('slug, sarlavha, kategoriya, bosqich, qisqa, daqiqa')
      .eq('yonalish', YONALISH).eq('faol', true).eq('bolim', bolim).order('sort_order').order('created_at')
      .then(({ data }) => { setDarslar((data ?? []) as KursDars[]); setLoading(false) })
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [bolim])
  ...
  return (
    <div style={{ maxWidth: '760px', margin: '0 auto', padding: '20px 20px 40px' }}>
      <h2 style={{ margin: '0 0 5px', fontSize: '22px', fontWeight: 800, display: 'flex', alignItems: 'center', gap: '9px' }}>
        <span style={{ color: 'var(--accent)' }}><BookOpen size={22} strokeWidth={2} /></span> {BOLIM_SARLAVHA[bolim] ?? 'Urologiya'}
      </h2>
      ...
      {loading ? (
        ...
        <div style={{ textAlign: 'center', padding: '50px 20px', color: 'var(--muted)' }}>
          <div style={{ display: 'flex', justifyContent: 'center', marginBottom: '10px', color: 'var(--accent)' }}><BookOpen size={38} strokeWidth={1.5} /></div>
          <p style={{ margin: 0 }}>Bu bo&apos;limda hozircha material yo&apos;q — tez orada qo&apos;shiladi.</p>
        </div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          {darslar.map((d) => (
            <button key={d.slug} onClick={() => router.push(`/student/urologiya/darslar/${d.slug}`)} className="soft-press"
              style={{ display: 'flex', alignItems: 'center', gap: '13px', width: '100%', textAlign: 'left', cursor: 'pointer', background: 'var(--surface)', border: '1px solid var(--line)', borderLeft: `4px solid var(--accent)`, borderRadius: '14px', padding: '14px 16px' }}>
...
export default function UroDarslar() {
  return (
