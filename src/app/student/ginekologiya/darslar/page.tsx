  ...
  useEffect(() => {
    const load = async () => {
      const { data } = await supabase.from('gin_darslar')
        ...
        .eq('faol', true).order('bosqich').order('sort_order').order('created_at')
    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg)', color: 'var(--ink)', paddingBottom: '90px' }}>
      ...
      <div style={{ maxWidth: '760px', margin: '0 auto', padding: '20px 20px 40px' }}>
        <h2 style={{ margin: '0 0 5px', fontSize: '22px', fontWeight: 800, display: 'flex', alignItems: 'center', gap: '9px' }}>
          <span style={{ color: 'var(--gyn)' }}><BookOpen size={22} strokeWidth={2} /></span> Ginekologiya darslari
        </h2>
        <p style={{ margin: '0 0 20px', color: 'var(--muted)', fontSize: '13.5px' }}>Bosqichma-bosqich darslar.</p>
        ...
        {loading ? (
          ...
          <div style={{ textAlign: 'center', padding: '50px 20px', color: 'var(--muted)' }}>
            ...
            <p style={{ margin: 0 }}>Ginekologiya darslari tez orada qo&apos;shiladi.</p>
          </div>
        ) : (
          BOSQICHLAR.map((b) => {
