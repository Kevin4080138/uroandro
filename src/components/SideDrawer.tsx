...
export function SideDrawer({
  ochiq, onYopish, en = 320, taraf = 'right', children,
}: {
  ochiq: boolean
  onYopish: () => void
  en?: number
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 200 }}>
      ...
      <div
        ...
        aria-modal="true"
        style={{
          position: 'absolute', top: 0, bottom: 0, width: `min(${en}px, 86vw)`,
          ...(chap ? { left: 0 } : { right: 0 }),
          background: 'var(--surface)',
          boxShadow: chap ? '8px 0 40px rgba(0,0,0,.28)' : '-8px 0 40px rgba(0,0,0,.28)',
          display: 'flex', flexDirection: 'column', overflowY: 'auto',
          transform: `translateX(${korin ? 0 : (chap ? -100 : 100)}%)`,
